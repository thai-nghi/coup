...
@pytest.mark.parametrize(
    ...
    pytest.param("Amiya", 2, 400, "User does not have enough coins", id="buy-fail-not-enough-coin"),
    pytest.param("unknown", 2, 401, "Unauthorized", id="buy-fail-not-logged-in")
    )
)
@pytest.mark.asyncio
async def test_buy_item(client_factory, user, item_id, expected_status, text, shop_items):
